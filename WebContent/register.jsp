...
<tr>
	<td>회원번호(자동발생)</td>
	<td><input type="text" id="id_custno" name="custno" value=<%= maxCustno %> readonly></td> <!--read only is make it  -->
</tr>
<tr>
	<td>회원성명</td>     
...
(()=>{
	...
	const frmReg = document.querySelector('#id_Regform');
	...
	btnRegMember.addEventListener('click', ()=>{
		...
		myIframe.src="member.jsp"
		
		frmReg.action= "member.jsp";   // form 데이터가 전달될 페이지를 설정 (URL)

		frmReg.target = "myIframe"; // iframe;
		
		frmReg.submit();  // action 에서 설정한 URL 을 호출함으로써 폼데이터를 전달하라.
